...
EXPORT void term_cmoveto(Terminal *term, int y, int x)
{
...
EXPORT void term_cmove(Terminal *term, int dy, int dx)
{
    TerminalBuffer *b = ACTIVE_BUFFER(term);
    term_cmoveto(term, b->cursor.y + dy, b->cursor.x + dx);
}

EXPORT void term_puttab(Terminal *term, int count, int inc)
{
    TerminalBuffer *b = ACTIVE_BUFFER(term);
...
EXPORT void term_putc(Terminal *term, Cell cell)
{
    TerminalBuffer *b = ACTIVE_BUFFER(term);
    if (b->cursor.x == b->cols) {
        if (b->cursor.y == b->rows - 1)
            buffer_scrollup(b, 1);
        term_cmoveto(term, b->cursor.y + 1, 0);
    }
...
EXPORT void term_linefeed(Terminal *term)
{
    TerminalBuffer *b = ACTIVE_BUFFER(term);
...
EXPORT void term_cursor_save(Terminal *term)
{
    int buf                 = IS_SET(term->mode, MODE_ALT_BUFFER);
    term->saved_cursor[buf] = term->buffer[buf].cursor;
}

EXPORT void term_cursor_restore(Terminal *term)
{
    int buf                  = IS_SET(term->mode, MODE_ALT_BUFFER);
